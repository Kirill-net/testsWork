def dict_curs():
	...
	for x, y, z in zip(courses,mentors,durations):
	  course_dict = {"title": x, "mentors": y,"duration": z}
	...
	maxes = []
	minis = []
	for idx, duretion in enumerate(durations):
		if max_ == duretion :
			maxes.append(idx)
		elif min_ == duretion :
			minis.append(idx)
	...
	return (f'Самый короткий курс(ы): {courses_min} - {min_} месяца(ев)\n'
			f'Самый длинный курс(ы): {courses_max} - {max_} месяца(ев)')
